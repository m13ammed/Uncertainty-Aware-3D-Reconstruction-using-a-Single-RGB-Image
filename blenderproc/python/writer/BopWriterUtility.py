...
def write_bop(output_dir: str, target_objects: Optional[List[MeshObject]] = None,
              ...
              ignore_dist_thres: float = 100., m2mm: bool = True, frames_per_chunk: int = 1000, room_id = "example", instances = None):
    """Write the BOP data
    ...
    chunks_dir = os.path.join(dataset_dir, room_id)
    ...
    _BopWriterUtility.write_frames(chunks_dir, dataset_objects=dataset_objects, depths=depths, colors=colors,
                                   ...
                                   depth_scale=depth_scale, jpg_quality=jpg_quality, instances = instances)
...
class _BopWriterUtility:
    ...
    def get_frame_gt(dataset_objects: List[bpy.types.Mesh], unit_scaling: float, ignore_dist_thres: float,
                     destination_frame: Optional[List[str]] = None, obj_id = None):
        ...
        for obj in dataset_objects:
            ...
            if not np.linalg.norm(list(cam_t_m2c)) > ignore_dist_thres and obj.get_cp("instance_id") in obj_id: #and list(cam_t_m2c)[-1] > 0 
                cam_t_m2c = list(cam_t_m2c * unit_scaling)
                frame_gt.update(
                    {obj.get_cp("instance_id") : {
    ...
    def write_frames(chunks_dir: str, dataset_objects: list, depths: Optional[List[np.ndarray]] = None,
                     ...
                     ignore_dist_thres: float = 100., save_world2cam: bool = True, jpg_quality: int = 95, instances = None):
        ...
        rgb_tpath = os.path.join(chunks_dir,  'rgb', '{im_id:06d}' + '{im_type}')
        depth_tpath = os.path.join(chunks_dir, 'depth', '{im_id:06d}' + depth_ext)
        instances_path = os.path.join(chunks_dir, 'mask', '{im_id:06d}' + depth_ext)
        chunk_camera_tpath = os.path.join(chunks_dir, 'scene_camera.json')
        chunk_gt_tpath = os.path.join(chunks_dir,  'scene_gt.json')
        ...
        if len(chunk_dirs):
            ...
            last_chunk_gt_fpath = os.path.join(chunks_dir, 'scene_gt.json')
        ...
        if curr_frame_id != 0:
            ...
            chunk_gt = _BopWriterUtility.load_json(
                chunk_gt_tpath, keys_to_int=True)
            chunk_camera = _BopWriterUtility.load_json(
                chunk_camera_tpath, keys_to_int=True)
        ...
        for frame_id in range(bpy.context.scene.frame_start, bpy.context.scene.frame_end):
            ...
            if curr_frame_id == 0:
                ...
                os.makedirs(os.path.dirname(
                    rgb_tpath.format( im_id=0, im_type='PNG')))
                os.makedirs(os.path.dirname(
                    depth_tpath.format(im_id=0)))
                os.makedirs(os.path.dirname(
                    instances_path.format(im_id=0)))
            ...
            chunk_gt[curr_frame_id] = _BopWriterUtility.get_frame_gt(dataset_objects, unit_scaling, ignore_dist_thres, obj_id = np.unique(instances[frame_id])) #edit this
            ...
            if colors:
                ...
                if color_file_format == 'PNG':
                    rgb_fpath = rgb_tpath.format( im_id=curr_frame_id, im_type='.png')
                    cv2.imwrite(rgb_fpath, color_bgr)
                elif color_file_format == 'JPEG':
                    rgb_fpath = rgb_tpath.format(im_id=curr_frame_id, im_type='.jpg')
                ...
                rgb_fpath = rgb_tpath.format(im_id=curr_frame_id, im_type=color_ext)
            ...
            instance =   instances[frame_id] 
            cv2.imwrite(instances_path.format(im_id=curr_frame_id), instance)
            ...
            depth_fpath = depth_tpath.format(im_id=curr_frame_id)
            ...
            if ((curr_frame_id + 1) % frames_per_chunk == 0) or \
                ...
                # Save GT annotations.
                _BopWriterUtility.save_json(chunk_gt_tpath, chunk_gt)

                # Save camera info.
                _BopWriterUtility.save_json(chunk_camera_tpath, chunk_camera)
